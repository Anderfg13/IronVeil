...
>
> **Última actualización:** 2026-09-22 · **Por:** García
...
7. **V5 (agotamiento de recursos) por niveles graduados de concurrencia, C0 vs. C5
   ...
   sature, aunque no elimina el efecto del hardware débil.** Script nuevo
   `ataques/vector5_agotamiento.py` (reutiliza `ataques/vector5_carga.py`,
   no duplica lógica de HTTP/carga), niveles 10/50/100 peticiones
   concurrentes, 20s por nivel, contra el mismo hardware descrito en la
   sección 7. **C0:** ya en el nivel más bajo (10 concurrentes) 9/10
   peticiones agotan el timeout de 90s (p50 90612ms); en el nivel 100,
   100/100 (p50 91163ms) — el servicio no se "cae" (no hay 5xx del proxy
   mismo), pero deja de responder dentro de un tiempo razonable para
   cualquier cliente real, ya con la concurrencia más baja probada.
   **C5:** en el nivel 10 el comportamiento es indistinguible de C0 (9/10
   timeouts, p50 90603ms) porque la ráfaga completa de 10 peticiones cabe
   dentro de la ventana de 60s del límite de 10/min y nunca lo dispara;
   en los niveles 50 y 100, el límite sí se dispara y la inmensa mayoría de
   peticiones (720/730 y 685/695) quedan bloqueadas (429) en vez de
   colgarse contra Ollama, con latencia p50 de 581ms y 1412ms — dos
   órdenes de magnitud más rápida que un timeout. **Lectura honesta, sin
   forzar hacia la hipótesis (regla 5 de `CLAUDE.md`):** el mecanismo sí
   cumple su función (excedente a la cola, no a Ollama), pero el punto de
   degradación medido de C0 (≤10 concurrentes) es mucho más bajo de lo que
   los niveles elegidos (10/50/100) podían resolver con detalle — no hay
   forma, con estos datos, de saber si el punto real está en 2, 5 u 8
   concurrentes, solo que ya está por debajo de 10. Esto es consecuencia
   directa de correr un modelo de lenguaje sobre CPU sin GPU dedicada
   (sección 7), no una propiedad del diseño de los mecanismos. **Costo de
   `aprobacion_humana` no capturado por esta ráfaga:** el 429 llega casi de
   inmediato porque `enviar_a_revision()` no espera a que un humano decida;
   el tiempo real que un usuario final esperaría hasta que alguien revise
   la cola (`tiempo_revision_humana_ms`, ver interfaz de Piedrahita) es un
   costo distinto, declarado como limitación, no medido aquí. Ver
   `resultados/2026-09-22/vector5_agotamiento_{C0,C5}_resumen.json`.
