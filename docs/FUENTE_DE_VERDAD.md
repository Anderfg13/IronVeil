...
| 2026-09-25 | Primer lote de issues de SonarCloud (seguridad y fiabilidad), tras activar el análisis en el repo. `CLAUDE.md` gana una sección de seguridad (7, "Seguridad del código (SonarCloud en A)") con la regla explícita de que aplica al código de infraestructura, no a los canarios/payloads de ataque (esos son el objeto de estudio, no una vulnerabilidad). **Path traversal (CWE-22):** `validar_ruta_salida_segura()` nueva en `vector5_carga.py` y `consolidar.py`, aplicada a `--salida`/`--salida-dir`/`--output-dir` en los 5 scripts que escriben archivos (restringe a la raíz del repo). `agregar_resultados_desde_jsonl.py` se deja **sin restringir a propósito** — lee evidencia de donde sea por diseño explícito, y los tests ya la ejercitan así con archivos fuera del repo; documentado en el código para marcar "won't fix" en SonarCloud en vez de romper el propósito del script. **SSRF (CWE-918):** `ejecutar_rafaga()` revalida el host contra `validar_host_laboratorio_propio()` en el punto real donde se dispara la petición de red, no solo en `main()` — defensa en profundidad para el otro punto de entrada (`vector5_agotamiento.ejecutar_niveles()`). **Dockerfile:** `--only-binary :all:` en ambos `pip install` (evita ejecutar `setup.py` de un sdist), usuario sin privilegios (`appuser`, antes corría como root) — **no probado con un build real** (Docker Desktop no estaba corriendo en este entorno y, dado el historial de corrupción de disco de Docker/Podman en sesiones anteriores, no se intentó levantarlo; pendiente que alguien del equipo corra `docker compose up --build` antes de confiar en el cambio). Hash-locking completo de dependencias (`--require-hashes`) quedó **sin aplicar**, mismo motivo. **Log Injection (CWE-117):** `_sanear_para_log()` nueva en `proxy/main.py`, aplicada a los 7 logs que incluían `modelo` (dato de la petición del cliente) sin sanear. **Fiabilidad:** `check-docs-before-commit.sh` con `[[` en vez de `[` (5 sitios) y `case` con default explícito. `_paso_filtrado()`/`_paso_minimo_privilegio()` ("async sin await" según SonarCloud): revisado y **documentado como falso positivo, no corregido** — quitar `async` rompe el contrato de tipo `PasoCadena` que `_CADENA_MECANISMOS` necesita para tratar los 3 pasos de forma uniforme. 229 tests (11 nuevos), ruff y black limpios. Quedan pendientes de esta misma tanda de SonarCloud: constantes duplicadas en `analisis/comparar_v3_c1_c2_c3.py`/`consolidar.py`, complejidad cognitiva en `comparar_v5_c0_c5.py` (de Fiquitiva), documentación de `HTTPException` en FastAPI, `logging.exception()` en `mecanismos.py`, y asserts compuestos en `tests/test_main.py`. | García | `CLAUDE.md`, `.claude/hooks/check-docs-before-commit.sh`, `analisis/agregar_resultados_desde_jsonl.py`, `analisis/consolidar.py`, `ataques/vector4_movimiento_lateral.py`, `ataques/vector5_agotamiento.py`, `ataques/vector5_carga.py`, `ataques/vectores_1_2_3.py`, `proxy/Dockerfile`, `proxy/main.py`, `tests/test_consolidar.py`, `tests/test_main.py`, `tests/test_vector5_carga.py` |
